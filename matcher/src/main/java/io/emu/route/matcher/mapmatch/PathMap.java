...
public class PathMap {
	...
	private void init(List<List<CandidatePath>> paths) {
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < csize; j++) {
				...
				// }
				String enodeid = (i + 1) + "@"
						+ cpath.getEd().getLink().getId();

				// System.out.println(cpath.getSt().getLinkID()+"->"+cpath.getEd().getLinkID()+":"+cpath.getPathsLength());
				// double
				// cost=-(Math.log(cpath.getCost())+Math.log(cpath.getEd().cost));
				// double pj =
				// cpath.getSt().getPrjDistance()+cpath.getEd().getPrjDistance();
				// double cost =
				// (cpath.getPathsLength()<0.0001?(pj):pj/cpath.getPathsLength()
				...
				if (i == 0) {// 处理第一个GPS点
					...
					// double
					// snodeCost=cpath.getSt().getLinkLenth()-cpath.getSt().getPrjDistanceFormSNode();
	...
	private List<CandidatePath> sortBestCaPath(List<CandidatePath> caPathList/**
	 * 
	 * ,int top
	 */
	) {
		Collections.sort(caPathList, new Comparator<CandidatePath>() {
			public int compare(CandidatePath o1, CandidatePath o2) {
				// double[] o2v = o2.getValue();
				// double[] o1v = o1.getValue();
				// double t1 = o1.getCost();
				// double t2 = o2.getCost();
				double t1 = o1.getPathLength() + o1.getSt().getPrjDistance()
						+ o1.getEd().getPrjDistance();
				double t2 = o2.getPathLength() + o2.getSt().getPrjDistance()
						+ o2.getEd().getPrjDistance();
				return (t1 > t2 ? 1 : -1);
			}
		});
