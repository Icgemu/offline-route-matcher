...
public class MapsObject {
	private Jedis jedis = new Jedis("172.16.52.6");
	private Neo4JAstar astar = null;

	public MapsObject(String dirs) {
		try {

			GraphDatabaseService graphDb = new GraphDatabaseFactory()
					.newEmbeddedDatabaseBuilder(new File(dirs))
					// .newEmbeddedDatabaseBuilder(new
					// File("D:/map/neo4jdata/map"))
					.newGraphDatabase();
	 ...
	 *            原始从文件读取的GPS数据
	 */
	public void setAzimuth(List<GPS> trace) {
		...
		for (int i = 0; i < trace.size(); i++) {
			...
			// 用两点连线的方位角提点GPS的角度
			if (i > 0) {
				// double azm = MapUtil.azimuth(lastgps.longtitude,
				// lastgps.latitude, gps.longtitude, gps.latitude);
				double azm = 0.0;
				double dist = MapUtil.calPointDistance(header.longtitude,
						header.latitude, gps.longtitude, gps.latitude);
				// double dist = MapUtil.calPointDistance(lastgps.longtitude,
				// lastgps.latitude, gps.longtitude, gps.latitude);
				// double spd = dist*3.6/((gps.getTime() -
				// lastgps.getTime())/1000);
				...
				if (dist < 25) {
					...
					if (headerazm == Double.MAX_VALUE) {
						azm = MapUtil.azimuth(lastgps.longtitude,
								lastgps.latitude, gps.longtitude, gps.latitude);
					} else {
						azm = headerazm;
					}
					...
					azm = MapUtil.azimuth(header.longtitude, header.latitude,
							gps.longtitude, gps.latitude);
					headerazm = azm;
					header = gps;
				}
				// if(i>1){
	...
	public void getCandidateLinks(GPS gps) {

		Point point = CellTools.createPoint(gps.getLongtitude(),
				gps.getLatitude());
		...
		List<Link> links = CellTools.getLinks(gps.getLongtitude(),
				gps.getLatitude(), jedis);
		...
		for (Link link : links) {
			...
			// 投影点
			Coordinate cs = CellTools
					.closestPoint2LineString(point, lineString);
			// Point prjp = CellTools.createPoint(cs);

			// 投影距离
			double prjDistance = CellTools.calPointDistance(
					point.getCoordinate(), cs);
			...
			for (int i = 0; i < size; i++) {// 计算投影点到Link起点距离
				...
				distance = CellTools.calPointDistance(coordinates[i],
						coordinates[i + 1]);
			...
			// 投影点所在段的方位角
			azimuth = MapUtil.azimuth(coordinates[choose],
					coordinates[choose + 1]);
			...
			if (link.isDual() && azimuthDelta > 90) {
				...
				finalLink = new Link("-" + link.getId(), link.isDual(),
						link.getEnodeID(), link.getSnodeID(), link.getLength(),
						(LineString) link.getLine().reverse());
				prjDistanceFormSNode = link.getLength() - prjDistanceFormSNode;
				if (prjDistanceFormSNode < 0) {
					prjDistanceFormSNode = 0.0;
				}
			}

			// 夹角过大,投影距离过大，剔除
			if (azimuthDelta > 90 || prjDistance > 50) {
				// if (azimuthDelta > 90) {
				continue;
			}
			...
			candidates.add(new CandidatePrj(finalLink, gps, prjDistance,
					prjDistanceFormSNode, azimuth, azimuthDelta, cost, cs,
					isInBound));
		}
	...
	private void adjust(GPS gps, List<CandidatePrj> prjs, int c) {
	 ...
	 * @throws Exception
	 */
	public List<List<CandidatePath>> getRoute(List<GPS> trace) throws Exception {
		// 设置方位角
		setAzimuth(trace);
		...
		for (int i = 1; i < trace.size(); i++) {
			...
			if (lastGps.getCandidates().size() < 1
					|| lastGps.getCandidates() == null) {
				lastGps = trace.get(i);
				continue;
			}
	 ...
	 * @throws IOException
	 */
	public void output(List<List<CandidatePath>> paths, String outfile)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	public List<String> output(List<List<CandidatePath>> paths)
			throws IOException {
		...
		for (List<CandidatePath> path : paths) {
			...
			for (CandidatePath p : path) {
				...
				String r = p.getSt().getGps().getTime()
						+ ","
						+ (p.getSt().getLink().getLength() - Math.round(p
								.getSt().getPrjDistanceFormSNode())) + ","
						+ p.getSt().getLink().getId() + ","
	 ...
	 * @throws IOException
	 */
	public List<String> output1(List<List<CandidatePath>> paths)
			throws IOException {
		// BufferedWriter bw = new BufferedWriter(new FileWriter(new
		// File(outfile) ));
		List<String> res = new ArrayList<String>();
		...
		for (List<CandidatePath> path : paths) {
			...
			for (CandidatePath p : path) {
				long t = p.getEd().getGps().getTime()
						- p.getSt().getGps().getTime();

				long len = 0l;

				if (p.getSt().getLink().getId()
						.equalsIgnoreCase(p.getEd().getLink().getId())) {
				...
				double spd = len * 1.0 / t;
				float soc = Math.abs(p.getSt().getGps().getSoc()
						- p.getEd().getGps().getSoc());

				double delta = soc / t;

				// System.out.println("t="+t+", len="+len+",spd="+spd+",soc="+soc+",delta="+delta);

				long startT = p.getSt().getGps().getTime()
						- Math.round(p.getSt().getPrjDistanceFormSNode() / spd);

				double startSoc = (p.getSt().getGps().getTime() - startT)
						* delta + p.getSt().getGps().getSoc();

				long l = Math.round((p.getSt().getLink().getLength() - p
						.getSt().getPrjDistanceFormSNode()));
				if (l < 0) {
					l = 0;
				}
				long endT = p.getSt().getGps().getTime() + Math.round(l / spd);

				double endSoc = p.getSt().getGps().getSoc()
						- (endT - p.getSt().getGps().getTime()) * delta;

				res.add(p.getSt().getLink().getId() + "," + startT + "," + endT
						+ "," + startSoc + "," + endSoc);
				...
				if (p.getPath() != null && p.getPath().size() > 0) {
					...
					for (Link lnk : p.getPath()) {
						String id = lnk.getId();
						long mybaseT = baseT
								+ Math.round(lnk.getLength() / spd);
						double mybaseSoc = baseSoc - (lnk.getLength() / spd)
								* delta;

						res.add(id + "," + baseT + "," + mybaseT + ","
								+ baseSoc + "," + mybaseSoc);
				...
				// endSoc = baseSoc + (p.getEd().getLink().getLength()/spd) *
				// delta;
				// res.add(p.getEd().getLink().getId()+ ","+ baseT
				// +","+endT+","+baseSoc+","+endSoc);
				// }
			}
	...
	public List<List<CandidatePath>> truncate(List<List<CandidatePath>> paths)
			throws IOException {
		// BufferedWriter bw = new BufferedWriter(new FileWriter(new
		// File(outfile) ));
		...
		for (List<CandidatePath> path : paths) {
			...
			for (int i = 1; i < path.size(); i++) {
				...
				if (pre.getSt().getGps().getSoc() == pre.getEd().getGps()
						.getSoc()
						&& pre.getEd()
								.getLink()
								.getId()
								.equalsIgnoreCase(cur.getSt().getLink().getId())) {
					pre.setEd(cur.getEd());
					...
					if (!cur.getSt().getLink().getId()
							.equalsIgnoreCase(cur.getEd().getLink().getId())) {
						if (!cur.getSt()
								.getLink()
								.getId()
								.equalsIgnoreCase(pre.getSt().getLink().getId()))
	...
	public String concatLink(List<Link> links) {
	...
	private boolean checkBreak(List<List<CandidatePath>> paths,
			List<CandidatePath> cur) {
		if (paths.size() < 1) {
			return false;
		}
		if (cur.size() < 1) {
	...
	private List<CandidatePath> getFinalPathFor(List<List<CandidatePath>> paths) {
		// Astar
		...
		for (int i = 0; i < map.getStartNodes().size(); i++) {
			for (int j = 0; j < map.getEndNodes().size(); j++) {
				List<String> nodes = astar.find(map,
						map.getStartNodes().get(i), map.getEndNodes().get(j));
		...
		for (int i = 1; i < finalNodes.size(); i++) {
			finalPath
					.add(map.getPath(finalNodes.get(i - 1), finalNodes.get(i)));
		}
	 ...
	 * @param path
	 */
	private void ajustWeightForPath(CandidatePath path) {
		...
		if (slinkId.equalsIgnoreCase(elinkId)) {
			path.setPathLength(ed.getPrjDistanceFormSNode()
					- st.getPrjDistanceFormSNode());
		} else {
			path.setPathLength(path.getPathLength() + st.getLink().getLength()
					- st.getPrjDistanceFormSNode()
					+ ed.getPrjDistanceFormSNode());
	 ...
	 * @return
	 */
	private List<CandidatePath> getPaths(List<CandidatePrj> s,
			List<CandidatePrj> e) {
			...
			try {
				...
				for (int i = 0; i < s.size(); i++) {
					for (int j = 0; j < e.size(); j++) {
						...
						boolean sameLink = slink.getId().equalsIgnoreCase(
								elink.getId());
						boolean jointLink = slink.getEnodeID().longValue() == elink
								.getSnodeID().longValue();
						if ((!sameLink) && (!jointLink)) {// 前后两个点的候选Link不同并且没有相连，需要补全路径
							String mid = slink.getEnodeID() + "_"
									+ elink.getSnodeID();
							...
							if (!id2cmp.containsKey(mid)) {
								...
								for (Relationship r : wp.relationships()) {
									...
									Integer len = (Integer) r
											.getProperty("length");
									Long sn = (Long) r.getStartNode()
											.getProperty("id");
									Long en = (Long) r.getEndNode()
											.getProperty("id");
									lnks.add(new Link(id, false, sn, en, len,
											null));
								}
								CandidatePath path = new CandidatePath(
										s.get(i), e.get(j), lnks, w, w);
								...
								CandidatePath path = new CandidatePath(
										s.get(i), e.get(j), cache.getPath(),
										cache.getCost(), cache.getCost());
								ajustWeightForPath(path);
							...
							CandidatePath path = new CandidatePath(s.get(i),
									e.get(j), null, 0d, Double.MAX_VALUE);
							ajustWeightForPath(path);
							if (!filterPath(path)) {
								paths.add(path);
							}
	 ...
	 * @return
	 */
	private boolean filterPath(CandidatePath p) {
