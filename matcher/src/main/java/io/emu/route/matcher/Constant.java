...
public class Constant {
	...
	public static CostEvaluator<Double> costEvaluator = new CostEvaluator<Double>() {
		...
		public Double getCost(Relationship rel, Direction direct) {
			System.out.println(rel.getProperty("id") + "@"
					+ rel.getProperty("length"));
			totalCost = totalCost
					+ Double.parseDouble(rel.getProperty("length").toString());
			return totalCost;
		}
	...
	// double dx = (Double) node.getProperty( "x" ) - (Double) goal.getProperty(
	// "x" );
