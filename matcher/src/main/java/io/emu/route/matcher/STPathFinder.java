...
public class STPathFinder {
	...
	static EstimateEvaluator<Double> estimateEval = CommonEvaluators
			.geoEstimateEvaluator("y", "x");
	PathFinder<WeightedPath> astar = new AStarPath(
			PathExpanders.forDirection(Direction.OUTGOING), costEval,
			estimateEval);
	GraphDatabaseService graphDb = null;

	public STPathFinder(String neo4jDbPath) {
		astar = new AStarPath(PathExpanders.forDirection(Direction.OUTGOING),
	...
	public long[] getRelationship(long id) {
		Transaction tx = graphDb.beginTx();
		IndexManager index = graphDb.index();
		Index<Relationship> relationshipIndex = index
