...
public class AStarPath implements PathFinder<WeightedPath> {
	private final PathExpander<?> expander;
	private final CostEvaluator<Double> lengthEvaluator;
	private final EstimateEvaluator<Double> estimateEvaluator;
	private Metadata lastMetadata;

	// public AStarPath( RelationshipExpander expander, CostEvaluator<Double>
	// lengthEvaluator, EstimateEvaluator<Double> estimateEvaluator ) {
	...
	public WeightedPath findSinglePath(Node start, Node end) {
		...
		while (doer.hasNext()) {
			...
			if (doer.score.get(node.getId()).wayLength > 100000d) {// ��Ӧ�����ǵ�ͼƥ������,����10����ֹͣ����
				return null;
			}
			if (node.equals(end)) {
				// Hit, return path
				...
				Relationship rel = graphDb.getRelationshipById(doer.cameFrom
						.get(node.getId()));
				while (rel != null) {
					rels.addFirst(rel);
					node = rel.getOtherNode(node);
					Long nextRelId = doer.cameFrom.get(node.getId());
					rel = nextRelId == null ? null : graphDb
							.getRelationshipById(nextRelId);
				}
	...
	public Iterable<WeightedPath> findAllPaths(Node node, Node end) {
		...
		return path != null ? Arrays.asList(path) : Collections
				.<WeightedPath> emptyList();
	}
	...
	private class Doer extends PrefetchingIterator<Node> implements Path {
		...
		private final TreeMap<Double, Collection<Node>> nextNodes = new TreeMap<Double, Collection<Node>>();
		...
		private Node popLowestScoreNode() {
			...
			Node node = entry.getValue().isEmpty() ? null : entry.getValue()
					.iterator().next();
			if (node == null) {
				return null;
			}
