...
public class ImportCSV {
	...
	public static void importNode(String csvpath,String dbpath) throws IOException {
	...
	public static void importLink(String csvpath,String dbpath) throws IOException {
		GraphDatabaseService graphDb = new GraphDatabaseFactory().newEmbeddedDatabase(new File(dbpath));
        BufferedReader br =null;
        String line = "";  
        Transaction tx = graphDb.beginTx();
        try{
        	Index<Node> nodeIndex=graphDb.index().forNodes("NodeIdIndex");
        	Index<Relationship> relationshipIndex=graphDb.index().forRelationships("LinkIdIndex");
        	long i=0; 
        	br = new BufferedReader(new FileReader(csvpath));
        	while ((line = br.readLine()) != null) {
        		String[] s = line.split("@");
...
//                if(i>=600000 && i<800000)
                { 
	        		...
	        		Relationship direction = relationshipIndex.get("id", linkid).getSingle();
		            if(direction==null){
		        		direction = firstNode.createRelationshipTo(secondNode, Constant.Road.direction);	            
		            ...
		            if(dr.equalsIgnoreCase("d")){
		            	Long linkid2 =-linkid;
		            	Relationship direction2 = relationshipIndex.get("id", linkid2).getSingle();
		            	if(direction2==null){
				            direction2 = secondNode.createRelationshipTo(firstNode, Constant.Road.direction);	            
