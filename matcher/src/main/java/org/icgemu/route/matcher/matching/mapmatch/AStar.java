...
{
		protected class Path implements Comparable{
				public T point;
				public Double f;
				public Double g;
				public Path parent;
		...
		protected HashMap<T, Double> mindists;
		...
		protected Double f(Path p, T from, T to){
				Double cost=g(from, to);
				if(cost!=null){
					Double g =  cost + ((p.parent != null) ? p.parent.g : 0.0);
					Double h = h(from, to);
		...
		public List<T> compute(T start){
				try{
						...
						for(;;){
								...
								T last = p.getPoint();
								int n = Integer.parseInt(((String)last).split("@")[0]);
