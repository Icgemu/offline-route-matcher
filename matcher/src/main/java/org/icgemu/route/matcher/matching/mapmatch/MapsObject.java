...
//			}
	 ...
	 * @throws IOException
	 */
	public  void output(List<List<CandidatePath>> paths , String outfile) throws IOException{
	 ...
	 * @throws IOException
	 */
	public  List<String> output(List<List<CandidatePath>> paths ) throws IOException{
		//BufferedWriter bw = new BufferedWriter(new FileWriter(new File(outfile) ));
		...
		for(List<CandidatePath> path: paths){
			...
			for(CandidatePath p:path){
				...
				String r = p.getSt().getGps().getTime()+","
			     + (p.getSt().getLink().getLength()- Math.round(p.getSt().getPrjDistanceFormSNode()))+","
			     + p.getSt().getLink().getId()+","
	 ...
	 * @throws IOException
	 */
	public List<String> output1(List<List<CandidatePath>> paths ) throws IOException{
		//BufferedWriter bw = new BufferedWriter(new FileWriter(new File(outfile) ));
		List<String> res = new ArrayList<String>();
		...
		for(List<CandidatePath> path: paths){
			...
			for(CandidatePath p:path){
				...
				long l = Math.round((p.getSt().getLink().getLength()-p.getSt().getPrjDistanceFormSNode()));
				if(l<0){l=0;}
				...
				if(p.getPath() != null  && p.getPath().size() > 0){
					...
					for(Link lnk : p.getPath()){
						String id = lnk.getId();
						long mybaseT = baseT + Math.round(lnk.getLength() / spd);
...
//				}
	...
	public  List<List<CandidatePath>> truncate(List<List<CandidatePath>> paths ) throws IOException{
		//BufferedWriter bw = new BufferedWriter(new FileWriter(new File(outfile) ));
	...
	private  boolean checkBreak(List<List<CandidatePath>> paths,List<CandidatePath> cur){
	 ...
	 * @return
	 */
	private  List<CandidatePath> getPaths(List<CandidatePrj> s, List<CandidatePrj> e){
