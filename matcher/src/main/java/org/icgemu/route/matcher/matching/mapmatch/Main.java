...
public class Main {
	...
	public static void writeLineShp(List<GPS> trace,
		...
		if (featureSource instanceof SimpleFeatureStore) {
			...
			for(GPS gps: trace){
				...
				//LineString prj = new LineString(new Coordinate[]{new Coordinate(gps.getLongtitude(),gps.getLatitude()),});
				//for (CandidatePrj g : gps.getCandidates()) {
					
				if(id>0){
				LineString prj = new LineString(new Coordinate[]{
							new Coordinate(lastGps.getLongtitude(),lastGps.getLatitude()),
							new Coordinate(gps.getLongtitude(),gps.getLatitude())},new PrecisionModel(), 4326);
	...
	public static void writeMatchingShp(List<GPS> trace,
			String out) throws Exception {
		...
		if (featureSource instanceof SimpleFeatureStore) {
			...
			for(GPS gps: trace){
				...
				for (CandidatePrj g : gps.getCandidates()) {
					LineString prj = new LineString(new Coordinate[]{
							new Coordinate(gps.getLongtitude(),gps.getLatitude()),g.getPrjPoint()},new PrecisionModel(), 4326);
					//Geometry geometry = MapUtil.parseWktString(s[9]);
					
					Object[] obj = { g.getLink().getId(), g.getGps().getTime(),
							g.getGps().getSpeed(), g.getPrjDistance(),g.getPrjDistanceFormSNode(),g.getLink().getLength(),
							g.getAzimuth(), g.getAzimuthDelta(), g.getCost(),g.getLink().isDual(),prj };
					lst.add(featureBuilder.buildFeature((id++) + "", obj));
	...
	public static void writePathShp(List<List<CandidatePath>> paths,
			String out) throws Exception {
...
//							new Coordinate(gps.getLongtitude(),gps.getLatitude()),g.getPrjPoint()},new PrecisionModel(), 4326);
					...
					LineString prj = new LineString(new Coordinate[]{
							new Coordinate(gps.getLongtitude(),gps.getLatitude()),g.getSt().getPrjPoint()},new PrecisionModel(), 4326);
					...
					if(g.getPath()!=null){
						for(Link l : g.getPath()){
							...
							//lst.add(featureBuilder.buildFeature((id++) + "", robj));
						}
					}
					...
					gps = g.getEd().getGps();
					LineString prj2 = new LineString(new Coordinate[]{
							new Coordinate(gps.getLongtitude(),gps.getLatitude()),g.getEd().getPrjPoint()},new PrecisionModel(), 4326);
					//Geometry prjall = prj.union(prj2);
					
					Object[] obj1 = {pid ,g.getSt().getGps().getTime(),g.getEd().getGps().getTime(),"1",g.getPathLength(), ls};
					
					Object[] obj2 = {pid ,g.getSt().getGps().getTime(),g.getEd().getGps().getTime(),"2", -1.0d, prj};
					Object[] obj3 = {pid ,g.getSt().getGps().getTime(),g.getEd().getGps().getTime(),"2", -1.0d, prj2};
	...
	public static List<GPS> readFile(String file) throws IOException{
		...
		while(line != null ){
			...
			rst.add(new GPS(lon, lat,
					-1, tdae, 
					Double.parseDouble(bcs_vehspd),
					Double.parseDouble(ems_engspd),
					Integer.parseInt(bcm_keyst)));
	...
	public static void test2(){
		GPS g = new GPS(113.26005999999999574, 23.18110300000000024,	5.7567378116777475	,1440519133000l,42.75,2065.0,2	);
